...
MacroMicroTree::MacroMicroTree(c_size_t n, space_efficient_vector_2D<c_size_t>& g): n(n), g(g){
    par.resize(n, -1);
    depths.resize(n);
    height.resize(n);
    bct.resize(n);
    ...
    space_efficient_vector<c_size_t> jumpPointersBuckets(n);
    for (c_size_t i = 0; i < n; i++){
        if (jumpNodes[i] == i && depths[i]){
            jumpPointersBuckets[i] = 32 - __builtin_clz(depths[i]);
        }
    }
    jumpPointers = space_efficient_vector_2D<c_size_t> (jumpPointersBuckets);
    ...
    for (c_size_t i = 0; i < n; i++){
        if (jumpNodes[i] == i){
            if (depths[i]){
                ...
                c_size_t currSz = jumpPointers.size(i);
                for (c_size_t j = 1; j < currSz; j++){
                    c_size_t need = c_size_t(1) << (j - 1);
                    if (posInLadder + need >= ladders.size(currLadder)){
                        need -= ladders.size(currLadder) - posInLadder;
                        c_size_t nxtNode = par[ladders(currLadder, ladders.size(currLadder) - 1)];
        ...
        for (c_size_t x = 0; x < g.size(i); x++){
            ...
            if (neighbor != par[i] && jumpNodes[neighbor] == -1){
                ...
                if (shapeSeen.find(mask) == shapeSeen.end()){
                    shapepos = shapeIdx++;
                    //TODO: convert table to flattened 2D vector once 3D flattened vector support is added
...
c_size_t MacroMicroTree::init_dfs(c_size_t node, c_size_t parent){
    par[node] = parent;
    c_size_t tot_sz = 1;
    for (c_size_t x = 0; x < g.size(node); x++){
...
void MacroMicroTree::constructLadders(c_size_t node){
    ...
    for (c_size_t j = 0; j <= height[node]; j++){
        ...
        for (c_size_t x = 0; x < g.size(currNode); x++){
    ...
    for (c_size_t j = height[node]; j >= 0; j--){
        for (c_size_t x = 0; x < g.size(ladder[j]); x++){
